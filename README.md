...
connect to `/ws`, send `hello` with protocol version `7`, then `join_game`. Omitting `room_id` joins
...
Protocol v7 example client flow:

```json
{"type":"hello","protocol_version":7}
...
Repeated movement input is coalesced per tick while jump press edges are retained. Protocol v7 sends
the local player's full owner snapshot separately from remote players' animation and interpolation
state. Both profiles carry distance-driven gait phase and five revisioned animation channels:
locomotion, upper-body action, additive reaction, full-body override, and life override. Input
acknowledgements and detailed ground-probe diagnostics are owner-only. Remote players do not receive
another player's input acknowledgement, jump availability, or collision-probe internals.
The server serializes each recipient's snapshot once and queues the same encoded text for delivery.
Protocol v7 removes the old `sprint` boolean, client-authored `yaw`, mutually exclusive character
`state`, and single `action_layer` fields; versions 1–6 are rejected. Welcome includes the collision
profile and capsule geometry required for local collision prediction. Incoming client messages larger
...
- Characters use a server-side Bullet capsule controller and its configured native step height.
  Projectiles use swept-sphere queries so fast projectiles do not skip collisions between ticks, and
  the owner's capsule is excluded from the projectile sweep.
...
solver order, protocol-v7 snapshot profiles, ground probing, prediction/reconciliation, interpolation, and
...
The browser runtime includes eight-direction gait samples, local-neighbor blend-space selection,
quaternion skeleton/clip sampling, pose blending and transition inertialization, independent animation
channels, distance-driven gait phase, additive aim, geometry-driven foot support and locks, distributed
pose orientation warp, root-motion extraction/application, time-windowed target warping, solver-rollout
trajectories, pose history, and contact-aware brute-force motion matching. Owner input sampling uses a
bounded fixed-step scheduler and prediction starts after the first authoritative owner snapshot. The
Motion Lab runs these algorithms on a synthetic seven-bone rig and labeled synthetic pose database. It
does not provide a production WebGL renderer,
